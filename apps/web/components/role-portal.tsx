...
function useVersionCheck() {
  ...
  useEffect(() => {
    ...
    async function checkVersion() {
      try {
        // Versiya faylını və ya endpoint-i yoxla (public/version.json və ya api-dən)
      ...
      timer = setTimeout(checkVersion, 60000); // hər 1 dəqiqədən bir yoxla
    }
    checkVersion();
...
function UpdateBanner() {
  ...
  return (
    ...
    >
      ...
      <button style={{ marginLeft: 16 }} onClick={() => setVisible(false)}>
        Bağla
      </button>
    </div>
  );
...
export function RolePortal() {
  const showUpdate = useVersionCheck();
  ...
  const [patientPortal, setPatientPortal] = useState<PatientPortalData | null>(
    null,
  );
  ...
  async function loadAdminData() {
    ...
    const [metricData, appointmentData, patientData, doctorData, staffData] =
      await Promise.all([
        requestJson<MetricState>("/observability/metrics"),
  ...
  async function loadDoctorData() {
    const data = await requestJson<DoctorDashboardData>(
  ...
  async function loadDoctorPatientDetails(patientId: string) {
    ...
    const data = await requestJson<PatientDetails>(
  ...
  useEffect(() => {
    const savedToken =
      globalThis.localStorage?.getItem("hospital_portal_token") ?? "";
    if (savedToken) {
      setToken(savedToken);
  ...
  async function changePassword() {
    ...
    try {
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : t("messages.changePasswordError"),
      );
    }
  }
      ...
      const payload = await requestJson<{ token: string; user: CurrentUser }>(
        "/auth/register-patient",
        {
          ...
          body: JSON.stringify({
            ...registerForm,
            birthDate: new Date(
              `${registerForm.birthDate}T00:00:00.000Z`,
            ).toISOString(),
    ...
    try {
      await requestJson("/patients", {
        ...
        body: JSON.stringify({
          ...patientCreateForm,
          birthDate: new Date(
            `${patientCreateForm.birthDate}T00:00:00.000Z`,
          ).toISOString(),
  ...
  async function createStaff() {
    ...
    try {
      ...
      setMessage(
        t("messages.staffAccountCreated", { role: roleLabel(staffForm.role) }),
      );
  ...
  async function deleteStaff(id: string) {
    ...
    if (
      !confirm(
        "Bu işçini silmək istədiyinizə əminsiniz? Bu əməliyyat geri qaytarılmayacaq.",
      )
    ) {
      return;
    }
  ...
  async function deleteDoctor(id: string) {
    ...
    if (
      !confirm(
        "Bu həkimi silmək istədiyinizə əminsiniz? Bu əməliyyat geri qaytarılmayacaq.",
      )
    ) {
      return;
    }
  ...
  async function createAppointment(channel: "call-center" | "web" | "mobile") {
    ...
    try {
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : t("messages.appointmentCreateError"),
      );
    }
  }
  ...
  async function createPatientAppointment() {
    ...
    try {
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : t("messages.appointmentBookingError"),
      );
    }
  }
  ...
  async function createMedicalRecord() {
    ...
    try {
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : t("messages.medicalRecordCreateError"),
      );
    }
  }
  ...
  async function shareReport(method: "email" | "whatsapp") {
    ...
    setMessage(
      `${method === "email" ? "E-posta" : "WhatsApp"} ilə paylaşım xüsusiyyəti yaxında əlavə ediləcək.`,
    );
  }
      ...
      <article className="panel-card auth-panel">
        <div className="auth-switcher">
          ...
          <button
            ...
            onClick={() => setAuthMode("patient-register")}
          >
            {t("tabs.patientRegister")}
          </button>
          <button
            type="button"
            className={
        ...
        authMode === "admin-login" ? (
          <>
            <span className="eyebrow">{t("headings.secureLogin")}</span>
            <h2>
              {authMode === "patient-login"
                ? t("headings.patientPortal")
                : authMode === "admin-login"
                  ? "Admin Paneli"
                  : t("headings.staffPortal")}
            </h2>
            <p>
              {authMode === "admin-login"
                ? "Sadece yöneticiler için giriş sayfası"
                : t("descriptions.staffIntro")}
            </p>
          ...
          <>
            ...
            <div className="form-grid">
              ...
              <label>
                ...
                >
                  <option value="FEMALE">
                    {t("form.genderFemale", "Qadın")}
                  </option>
                  <option value="MALE">{t("form.genderMale", "Kişi")}</option>
                  <option value="OTHER">
                    {t("form.genderOther", "Digər")}
                  </option>
                </select>
              </label>
              <label>
        ...
        {authMode === "bootstrap" ? (
          <>
            <span className="eyebrow">{t("headings.bootstrap")}</span>
            <h2>{t("headings.bootstrap")}</h2>
            <div className="form-grid single">
              <label>
                {t("form.clinicName", "Klinika adı")}
                <input
                  value={bootstrapForm.clinicName}
                  onChange={(event) =>
        ...
        {authMode === "reset-password" ? (
          <>
            <span className="eyebrow">{t("headings.resetPassword")}</span>
            <h2>{t("headings.resetPassword")}</h2>
            <p>{t("descriptions.resetHint")}</p>
            <div className="form-grid single">
              <label>
                {t("form.setupKey")}
                <input
    ...
    return (
      <main className="admin-shell portal-shell">
          ...
          <nav className="sidebar-nav" aria-label="Rol menyusu">
            {navigation.map((item) => (
              <button
                ...
                type="button"
                className={
                  activeSection === item.key ? "nav-label active" : "nav-label"
                }
        ...
        <section className="dashboard">
          {loading ? (
            <p className="status-ok">{t("messages.loadingData")}</p>
          ) : null}
          {message ? <p className="status-ok">{message}</p> : null}
          {error ? <p className="status-error">{error}</p> : null}
          {body}
  ...
  function renderAdminDashboard() {
    ...
    const employeeUsers = adminUsers.filter(
      (item) => item.role !== "PATIENT" && item.role !== "DOCTOR",
    );
    const activeEmployees = employeeUsers.filter(
      (item) => item.active !== false,
    );
    const formerEmployees = employeeUsers.filter(
      (item) => item.active === false,
    );
        ...
        {section === "dashboard" ? (
          <section className="metrics-grid">
            <article className="metric-card">
              <span>{t("metrics.totalRequests")}</span>
              <strong>{metrics.totalRequests}</strong>
              <p>{t("metrics.liveData")}</p>
            </article>
            <article className="metric-card">
              <span>{t("metrics.totalErrors")}</span>
              <strong>{metrics.totalErrors}</strong>
              <p>{t("metrics.trace")}</p>
            </article>
            <article className="metric-card">
              <span>{t("metrics.averageResponseMs")}</span>
              <strong>{metrics.averageResponseMs} ms</strong>
              <p>{t("metrics.latency")}</p>
            </article>
            <article className="metric-card">
              <span>{t("metrics.p95")}</span>
          ...
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.staffSection")}</span>
                  <h2>{t("forms.newStaff")}</h2>
                </div>
              </div>
              <div className="form-grid">
                <label>
            ...
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.formerStaff")}</span>
                  <h2>{t("admin.activeAndInactiveStaff")}</h2>
                </div>
              </div>
              <div className="stack-list">
                {activeEmployees.map((item) => (
                  <div key={item.id} className="report-item">
                    ...
                    <p>
                      {roleLabel(item.role)} • {t("admin.active")}
                    </p>
                    <button
                      type="button"
                      className="ghost-button"
                      onClick={() => void deactivateStaff(item.id)}
                    >
                ...
                {formerEmployees.map((item) => (
                  <div key={item.id} className="report-item">
                    <strong>{item.email}</strong>
                    <p>
                      {roleLabel(item.role)} • {t("admin.formerEmployee")}
                    </p>
                    <button
                      type="button"
                      className="danger-button"
                      onClick={() => void deleteStaff(item.id)}
                    >
                      {t("buttons.delete")}
                    </button>
                  </div>
        ...
        {section === "doctors" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.doctorManagement")}</span>
                  <h2>{t("forms.newDoctor")}</h2>
                </div>
              </div>
              <div className="form-grid">
                <label>
                ...
                <label>
                  {t("form.roomNumber")}
                  <input
                    value={doctorCreateForm.roomNumber}
                    onChange={(event) =>
                      setDoctorCreateForm((current) => ({
                        ...current,
            ...
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.doctors")}</span>
                  <h2>{t("admin.activeAndFormerDoctors")}</h2>
                </div>
              </div>
              <div className="stack-list">
                {activeDoctors.map((doctor) => (
                  <div key={doctor.id} className="report-item">
                    ...
                    <p>
                      {doctor.branch}
                      {doctor.roomNumber
                        ? ` • ${t("admin.doctorRoom")} ${doctor.roomNumber}`
                        : ""}{" "}
                      • {t("admin.active")}
                    </p>
                    <button
                      type="button"
                      className="ghost-button"
                      onClick={() => void deactivateDoctor(doctor.id)}
                    >
                      {t("buttons.deactivate")}
                    </button>
                  </div>
        ...
        {section === "patients" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.patientFlow")}</span>
                  <h2>{t("admin.newPatientRegistration")}</h2>
                </div>
              </div>
              <div className="form-grid">
                <label>
                ...
                <label>
                  ...
                  <input
                    type="password"
                    value={patientCreateForm.password}
                    onChange={(event) =>
                      setPatientCreateForm((current) => ({
                        ...current,
                        password: event.target.value,
                ...
                <label>
                  {t("form.identityNumber")}
                  <input
                    value={patientCreateForm.identityNumber}
                    onChange={(event) =>
                      setPatientCreateForm((current) => ({
                        ...current,
                        identityNumber: event.target.value,
                ...
                <label>
                  {t("form.gender")}
                  <select
                    value={patientCreateForm.gender}
                    onChange={(event) =>
                      setPatientCreateForm((current) => ({
                        ...current,
                        gender: event.target.value,
                ...
                <label>
                  ...
                  <input
                    type="date"
                    value={patientCreateForm.birthDate}
                    onChange={(event) =>
                      setPatientCreateForm((current) => ({
                        ...current,
                        birthDate: event.target.value,
            ...
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.patientList")}</span>
                  <h2>{t("admin.recentRegistrations")}</h2>
                </div>
              </div>
              <div className="stack-list">
                {patients.slice(0, 10).map((patient) => (
                  <div key={patient.id} className="report-item">
                    <strong>{patient.fullName}</strong>
                    <p>
                      {patient.identityNumber} • {patient.phone}
                    </p>
                  </div>
                ))}
              </div>
        ...
        {section === "appointments" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.appointments")}</span>
                  <h2>{t("admin.upcomingSchedule")}</h2>
                </div>
              </div>
        ...
        {section === "settings" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.accountSettings")}</span>
        ...
        <header className="topbar">
          <div>
            <span className="eyebrow">
              {t("landing.cards.frontdesk.title")}
            </span>
            <h1>{t("forms.patientRegistration")}</h1>
            <p>{t("descriptions.staffIntro")}</p>
          </div>
          <div className="topbar-actions">
            <button type="button" onClick={() => void loadFrontdeskData()}>
              {t("buttons.refresh", "Yenilə")}
            </button>
          </div>
        </header>
        ...
        {section === "register" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.newPatient")}</span>
                  <h2>{t("descriptions.register")}</h2>
                </div>
              </div>
              <div className="form-grid">
          ...
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">
                    {t("admin.frontdeskAppointment")}
                  </span>
                  <h2>{t("admin.frontdeskAppointmentTitle")}</h2>
                </div>
              </div>
              <div className="form-grid">
                <label>
              ...
              <button
                type="button"
                onClick={() => void createAppointment("call-center")}
              >
                {t("buttons.addAppointment")}
              </button>
            </article>
          </section>
        ) : null}
        ...
        {section === "doctors" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("admin.todayFlow")}</span>
                  <h2>{t("admin.upcomingAppointments")}</h2>
                </div>
              </div>
              <div className="appointment-list">
                {appointments.map((appointment) => (
      ...
      <>
        <header className="topbar">
          <div>
            ...
            <p>
              {doctorDashboard?.doctor.branch}
              {doctorDashboard?.doctor.roomNumber
                ? ` • ${t("admin.doctorRoom")} ${doctorDashboard.doctor.roomNumber}`
                : ""}
            </p>
          </div>
          <div className="topbar-actions">
            <button type="button" onClick={() => void loadDoctorData()}>
              {t("buttons.refresh", "Yenilə")}
            </button>
          </div>
        </header>
        ...
        {section === "appointments" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("doctor.todaySchedule")}</span>
                  <h2>{t("doctor.myAppointments")}</h2>
                </div>
              </div>
              <div className="appointment-list">
                {doctorDashboard?.appointments.map((appointment) => (
                  <button
                    ...
                    onClick={() =>
                      setSelectedDoctorPatientId(appointment.patientId ?? "")
                    }
                  >
                    <div className="appointment-row full-row">
                      <div>
                        <strong>{appointment.patientName}</strong>
                        <p>
                          {appointment.identityNumber} •{" "}
                          {formatDate(appointment.startsAt)}
                        </p>
                      </div>
                      <div className="appointment-meta">
            ...
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("doctor.patientProfile")}</span>
                  <h2>
                    {doctorPatientDetails?.fullName ??
                      t("doctor.selectPatientPlaceholder")}
                  </h2>
                </div>
              </div>
              {doctorPatientDetails ? (
                <div className="detail-stack">
                  <div className="info-grid">
                    <div>
                      <span>{t("doctor.selectPatientPlaceholder")}</span>
                      <strong>{doctorPatientDetails.identityNumber}</strong>
                    </div>
                    <div>
                      <span>{t("buttons.refresh")}</span>
                      <strong>{doctorPatientDetails.phone}</strong>
                    </div>
                    <div>
                      <span>{t("doctor.bloodType")}</span>
                      <strong>{doctorPatientDetails.bloodType || "-"}</strong>
                    </div>
                    <div>
                      <span>{t("doctor.allergies")}</span>
                      <strong>{doctorPatientDetails.allergies || "-"}</strong>
                    </div>
                  </div>
                  <div className="stack-list">
                    <h3>{t("doctor.pastAppointments")}</h3>
                    {doctorPatientDetails.appointments.map((appointment) => (
                      <div key={appointment.id} className="report-item">
                        <strong>{appointment.doctorName}</strong>
                        <p>
                          {formatDate(appointment.startsAt)} •{" "}
                          {statusLabel(appointment.status)}
                  ...
                  <div className="stack-list">
                    ...
                    {doctorPatientDetails.medicalRecords.map((record) => (
                      <div key={record.id} className="report-item">
                        ...
                        <p>
                          {record.prescribedBy} • {record.treatmentPlan}
                        </p>
                      </div>
                    ))}
                  </div>
        ...
        {section === "records" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("doctor.medicalRecord")}</span>
                  <h2>{t("doctor.newReport")}</h2>
                </div>
              </div>
              <div className="form-grid single">
                <label>
                ...
                <label>
                  {t("doctor.reportType")}
                  <select
                    value={medicalRecordForm.reportType}
                    onChange={(event) =>
                      setMedicalRecordForm((current) => ({
                        ...current,
                  ...
                  >
                    <option value="general">{t("doctor.generalReport")}</option>
                    <option value="blood-test">{t("doctor.bloodTest")}</option>
                    <option value="x-ray">{t("doctor.xRay")}</option>
                    <option value="ultrasound">{t("doctor.ultrasound")}</option>
                    <option value="ecg">{t("doctor.ecg")}</option>
                    <option value="biopsy">{t("doctor.biopsy")}</option>
              ...
              <div className="form-actions">
                <button
                  type="button"
                  onClick={() => void createMedicalRecord()}
                >
                  {t("doctor.saveRecord")}
                </button>
                <button type="button" onClick={() => void downloadReport()}>
                  {t("doctor.downloadReport")}
                </button>
                <div className="share-buttons">
                  <button
                    type="button"
                    onClick={() => void shareReport("email")}
                  >
                    {t("doctor.shareViaEmail")}
                  </button>
                  <button
                    type="button"
                    onClick={() => void shareReport("whatsapp")}
                  >
                    {t("doctor.shareViaWhatsApp")}
                  </button>
                </div>
              </div>
            </article>
            ...
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("doctor.selectedPatient")}</span>
                  <h2>{t("doctor.medicalHistory")}</h2>
                </div>
              </div>
              <div className="stack-list">
                {doctorPatientDetails?.medicalRecords.map((record) => (
        ...
        {section === "patients" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("doctor.patientList")}</span>
                  <h2>{t("doctor.assignedPatients")}</h2>
                </div>
              </div>
              <div className="stack-list">
                {doctorDashboard?.patients.map((patient) => (
                  ...
                  >
                    <div className="report-item selectable-item">
                      <strong>{patient.fullName}</strong>
                      <p>
                        {patient.latestRecord?.diagnosis ??
                          t("doctor.noRecord")}
                      </p>
                    </div>
                  </button>
        ...
        {section === "settings" ? (
          <section className="content-grid lower-grid">
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">PDF Şablonu</span>
                  <h2>Hesabat Şablonu Ayarları</h2>
                </div>
              </div>
              <div className="form-grid">
                <label>
                  Xəstəxana Adı
                  <input
                    value={pdfTemplateForm.hospitalName}
                    onChange={(event) =>
                      setPdfTemplateForm((current) => ({
                        ...current,
                ...
                <label>
                  Ünvan
                  <input
                    value={pdfTemplateForm.hospitalAddress}
                    onChange={(event) =>
                      setPdfTemplateForm((current) => ({
                        ...current,
                ...
                <label>
                  Telefon
                  <input
                    value={pdfTemplateForm.hospitalPhone}
                    onChange={(event) =>
                      setPdfTemplateForm((current) => ({
                        ...current,
                        hospitalPhone: event.target.value,
                      }))
                    }
                  />
                </label>
                <label>
                  {t("doctor.hospitalLogo")}
                  <input
                    value={pdfTemplateForm.hospitalLogo}
                    onChange={(event) =>
                      setPdfTemplateForm((current) => ({
                        ...current,
                        hospitalLogo: event.target.value,
                      }))
                    }
                ...
                <label>
                  Həkim Ünvanı
                  <input
                    value={pdfTemplateForm.doctorTitle}
                    onChange={(event) =>
                      setPdfTemplateForm((current) => ({
                        ...current,
                ...
                <label>
                  Həkim İmza
                  <input
                    value={pdfTemplateForm.doctorSignature}
                    onChange={(event) =>
                      setPdfTemplateForm((current) => ({
                        ...current,
                        doctorSignature: event.target.value,
                      }))
                    }
                  />
                </label>
                <label>
                  Dil
                  <select
                    value={pdfTemplateForm.language}
                    onChange={(event) =>
                      setPdfTemplateForm((current) => ({
                        ...current,
              ...
              <label className="full-width-field">
                Alt Mətn (Footer)
                <textarea
                  value={pdfTemplateForm.footerText}
                  onChange={(event) =>
                    setPdfTemplateForm((current) => ({
                      ...current,
              ...
              <div className="form-actions">
                <button type="button" onClick={loadPDFTemplateSettings}>
                  Cari Ayarları Yüklə
                </button>
                <button type="button" onClick={savePDFTemplateSettings}>
            ...
            <article className="panel-card">
              <div className="panel-head">
                <div>
                  <span className="eyebrow">{t("doctor.accountSettings")}</span>
                  <h2>{t("doctor.changePassword")}</h2>
                </div>
              </div>
              <div className="form-grid single">
                <label>
                  {t("doctor.currentPassword")}
                  <input
                    type="password"
                    value={changePasswordForm.currentPassword}
                    onChange={(event) =>
      ...
      <>
        <header className="topbar">
          <div>
            ...
            <p>
              {patientPortal?.identityNumber} • {patientPortal?.phone}
            </p>
          </div>
          <div className="topbar-actions">
            <button type="button" onClick={() => void loadPatientData()}>
              {t("buttons.refresh", "Yenilə")}
            </button>
          </div>
        </header>
          ...
          <section className="content-grid lower-grid">
            {section === "booking" ? (
              <article className="panel-card">
                <div className="panel-head">
                  <div>
                    <span className="eyebrow">
                      {t("patient.newAppointment")}
                    </span>
                    <h2>{t("patient.selectDoctor")}</h2>
                  </div>
                </div>
                <div className="form-grid">
                  <label>
                    {t("patient.doctor")}
                    <select
                      value={patientBookingForm.doctorId}
                      onChange={(event) =>
                        setPatientBookingForm((current) => ({
                          ...current,
                          doctorId: event.target.value,
                  ...
                  <label>
                    ...
                    <input
                      type="datetime-local"
                      value={patientBookingForm.startsAt}
                      onChange={(event) =>
                        setPatientBookingForm((current) => ({
                          ...current,
                          startsAt: event.target.value,
                  ...
                  <label>
                    ...
                    <input
                      type="datetime-local"
                      value={patientBookingForm.endsAt}
                      onChange={(event) =>
                        setPatientBookingForm((current) => ({
                          ...current,
                          endsAt: event.target.value,
                ...
                <label className="full-width-field">
                  {t("patient.note")}
                  <textarea
                    value={patientBookingForm.notes}
                    onChange={(event) =>
                      setPatientBookingForm((current) => ({
                        ...current,
                        notes: event.target.value,
                ...
                <button
                  type="button"
                  onClick={() => void createPatientAppointment()}
                >
            ...
            {section === "profile" ? (
              <article className="panel-card">
                <div className="panel-head">
                  <div>
                    <span className="eyebrow">{t("patient.profile")}</span>
                    <h2>{t("patient.myMedicalInfo")}</h2>
                  </div>
                </div>
                <div className="info-grid">
                  <div>
                    <span>{t("doctor.bloodType")}</span>
                    <strong>{patientPortal?.bloodType || "-"}</strong>
                  </div>
                  <div>
                    <span>{t("doctor.allergies")}</span>
                    <strong>{patientPortal?.allergies || "-"}</strong>
                  </div>
                  <div>
                    <span>{t("doctor.chronicConditions")}</span>
                    <strong>{patientPortal?.chronicConditions || "-"}</strong>
                  </div>
                  <div>
                    <span>{t("patient.gender")}</span>
                    <strong>{patientPortal?.gender || "-"}</strong>
                  </div>
                </div>
              </article>
            ) : null}
              ...
              <article className="panel-card">
                <div className="panel-head">
                  <div>
                    ...
                    <h2>{t("patient.newAndActiveVisits")}</h2>
                  </div>
                </div>
                <div className="appointment-list">
            ...
            {section === "reports" ? (
              <article className="panel-card">
                <div className="panel-head">
                  <div>
                    <span className="eyebrow">
                      {t("patient.pastAndReports")}
                    </span>
                    <h2>{t("patient.previousVisitsAndReceipts")}</h2>
                  </div>
                </div>
                <div className="stack-list">
