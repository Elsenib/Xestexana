  
const az = {
  appTitle: "Hospital Platform",
  ui: {
    language: "Dil",
    selectOption: "Seçin",
    languages: {
      az: "Azərbaycan dili",
      en: "English",
      ru: "Русский",
      tr: "Türkçe",
    },
  },

  roles: {
    ADMIN: "Admin",
    DOCTOR: "Həkim",
    NURSE: "Tibb bacısı",
    CALL_CENTER: "Qeydiyyat",
    PATIENT: "Pasiyent",
  },
  auth: {
    tabs: {
      patientLogin: "Pasiyent girişi",
      patientRegister: "Pasiyent qeydiyyatı",
      staffLogin: "Personal girişi",
      bootstrap: "Admin qurulumu",
      resetPassword: "Şifrəni sıfırla",
    },
    headings: {
      patientPortal: "Pasiyent portalına giriş",
      staffPortal: "Personal panelinə giriş",
      bootstrap: "Yalnız ilk admin üçün",
      resetPassword: "Admin şifrəsini yenilə",
      secureLogin: "Təhlükəsiz giriş",
    },
    descriptions: {
      staffIntro:
        "Admin idarəetmə alətləri alır, həkim yalnız öz pasiyentlərini və randevularını görür, qeydiyyat masası pasiyent qəbulunu idarə edir.",
      register: "Onlayn hesab yaradın və randevularınızı idarə edin.",
      resetHint:
        "Qurulum açarını bilirsinizsə admin şifrəsini sıfırlaya bilərsiniz.",
    },
    buttons: {
      login: "Giriş et",
      logout: "Çıxış et",
      refresh: "Yenilə",
      createAccount: "Hesab yarat",
      bootstrap: "Admin hesabı yarat",
      resetPassword: "Şifrəni sıfırla",
      updatePassword: "Şifrəni dəyişdir",
      addDoctor: "Həkim əlavə et",
      addPatient: "Pasiyent əlavə et",
      addAppointment: "Randevu əlavə et",
      createStaff: "Personal yarat",
      deactivate: "Deaktiv et",
      delete: "Sil",
    },
    form: {
      email: "E-poçt",
      password: "Şifrə",
      identityNumber: "Şəxsiyyət nömrəsi",
      phone: "Telefon",
      firstName: "Ad",
      lastName: "Soyad",
      gender: "Cins",
      genderFemale: "Qadın",
      genderMale: "Kişi",
      genderOther: "Digər",
      birthDate: "Doğum tarixi",
      bloodType: "Qan qrupu",
      allergies: "Allergiyalar",
      chronicConditions: "Xroniki xəstəliklər",
      setupKey: "Qurulum açarı",
      currentPassword: "Cari şifrə",
      newPassword: "Yeni şifrə",
      confirmPassword: "Şifrə təsdiqi",
      role: "Rol",
      title: "Başlıq",
      branch: "Şöbə",
      roomNumber: "Otaq nömrəsi",
      patient: "Pasiyent",
      doctor: "Həkim",
      start: "Başlanğıc",
      end: "Son",
      notes: "Qeyd",
    },
  },
  form: {
    email: "E-poçt",
    password: "Şifrə",
    identityNumber: "Şəxsiyyət nömrəsi",
    phone: "Telefon",
    firstName: "Ad",
    lastName: "Soyad",
    gender: "Cins",
    genderFemale: "Qadın",
    genderMale: "Kişi",
    genderOther: "Digər",
    birthDate: "Doğum tarixi",
    bloodType: "Qan qrupu",
    allergies: "Allergiyalar",
    chronicConditions: "Xroniki xəstəliklər",
    setupKey: "Qurulum açarı",
    currentPassword: "Cari şifrə",
    newPassword: "Yeni şifrə",
    confirmPassword: "Şifrə təsdiqi",
    role: "Rol",
    title: "Başlıq",
    branch: "Şöbə",
    roomNumber: "Otaq nömrəsi",
    patient: "Pasiyent",
    doctor: "Həkim",
    start: "Başlanğıc",
    end: "Son",
    notes: "Qeyd",
  },
  tabs: {
    patientLogin: "Pasiyent girişi",
    patientRegister: "Pasiyent qeydiyyatı",
    staffLogin: "Personal girişi",
    bootstrap: "Admin qurulumu",
    resetPassword: "Şifrəni sıfırla",
  },
  headings: {
    patientPortal: "Pasiyent portalına giriş",
    staffPortal: "Personal panelinə giriş",
    bootstrap: "Yalnız ilk admin üçün",
    resetPassword: "Admin şifrəsini yenilə",
    secureLogin: "Təhlükəsiz giriş",
  },
  descriptions: {
    staffIntro:
      "Admin idarəetmə alətləri alır, həkim yalnız öz pasiyentlərini və randevularını görür, qeydiyyat masası pasiyent qəbulunu idarə edir.",
    register: "Onlayn hesab yaradın və randevularınızı idarə edin.",
    resetHint:
      "Qurulum açarını bilirsinizsə admin şifrəsini sıfırlaya bilərsiniz.",
  },
  buttons: {
    login: "Giriş et",
    logout: "Çıxış et",
    refresh: "Yenilə",
    createAccount: "Hesab yarat",
    bootstrap: "Admin hesabı yarat",
    resetPassword: "Şifrəni sıfırla",
    updatePassword: "Şifrəni dəyişdir",
    addDoctor: "Həkim əlavə et",
    addPatient: "Pasiyent əlavə et",
    addAppointment: "Randevu əlavə et",
    createStaff: "Personal yarat",
    deactivate: "Deaktiv et",
    delete: "Sil",
  },
  landing: {
    heroTitle: "Hər rol üçün ayrı, məqsədə uyğun iş ekranı",
    heroDescription:
      "Pasiyent randevu və tibbi qeydlərini izləyir, həkim yalnız öz pasiyentlərini görür, qeydiyyat masası isə kimlik yoxlayıb doğru həkimə yönləndirmə aparır.",
    stats: {
      portals: "Ayrı portal",
      auth: "Rol əsaslı giriş",
      live: "API və qeyd axını",
    },
    cards: {
      admin: {
        title: "Admin",
        description:
          "Sistem idarəsi, işçi hesabları, həkim və qeydiyyat nəzarəti.",
      },
      frontdesk: {
        title: "Qeydiyyat",
        description:
          "Kimlik yoxlama, pasiyent açılışı, uyğun həkimə çakışmasız qeyd.",
      },
      doctor: {
        title: "Həkim",
        description:
          "Günün randevuları, pasiyent profili, müalicə və təyinat qeydi.",
      },
      patient: {
        title: "Pasiyent",
        description:
          "Yeni randevu, keçmiş görüşlər, rapor və reseptlərin görünüşü.",
      },
    },
  },
  sidebar: {
    dashboard: "Dashboard",
    staff: "İşçi hesabları",
    doctors: "Həkimlər",
    patients: "Pasiyentlər",
    appointments: "Randevular",
    settings: "Tənzimləmələr",
    register: "Pasiyent qeydiyyatı",
    doBooking: "Randevu planlama",
    doctorFlow: "Həkim axını",
    profile: "Profil",
    history: "Keçmiş görüşlər",
    reports: "Rapor və reseptlər",
  },
  shell: {
    adminTitle: "Admin panel",
    adminSubtitle: "Sistem idarəetməsi",
    deskTitle: "Qeydiyyat paneli",
    deskSubtitle: "Kimlik yoxlama və yönləndirmə",
    doctorTitle: "Həkim paneli",
    doctorSubtitle: "Günün pasiyent axını",
    patientTitle: "Pasiyent portalı",
    patientSubtitle: "Şəxsi randevu və rapor görünüşü",
    activeSession: "Aktiv sessiya",
    roleField: "Rol",
  },
  admin: {
    description:
      "Isçi hesablarını açın, yeni həkim əlavə edin, qəbul axınını izləyin.",
    topbarEyebrow: "Moderator fonksiyaları",
    topbarTitle: "Admin idarəetmə paneli",
    staffSection: "İşçi idarəetməsi",
    formerStaff: "Əvvəlki işçilər",
    formerEmployee: "Keçmiş işçi",
    activeAndInactiveStaff: "Aktiv və deaktiv işçilər",
    doctorManagement: "Həkim idarəçiliyi",
    doctors: "Həkimlər",
    appointments: "Randevular",
    activeAndFormerDoctors: "Aktiv və keçmiş həkimlər",
    doctorRoom: "Həkim otağı",
    upcomingSchedule: "Gələn cədvəl",
    active: "Aktiv",
    deactivated: "Deaktiv edilmiş",
    patientFlow: "Pasiyent axını",
    newPatientRegistration: "Yeni pasiyent qeydiyyatı",
    patientList: "Pasiyent siyahısı",
    recentRegistrations: "Son qeydiyyatlar",
    newPatients: "Yeni pasiyentlər",
    frontdeskAppointment: "Qeydiyyat randevuları",
    frontdeskAppointmentTitle: "Qeydiyyat üçün randevu yaradın",
    todayFlow: "Bugünkü axın",
    upcomingAppointments: "Gələn randevular",
    accountSettings: "Hesab tənzimləmələri",
    changePassword: "Şifrəni dəyişdir",
    newPatient: "Yeni pasiyent",
  },
  metrics: {
    totalRequests: "Toplam sorğu",
    totalErrors: "Xəta sayı",
    averageResponseMs: "Orta cavab",
    p95: "P95",
    liveData: "Canlı API statistikası",
    trace: "İzləmə paneli",
    latency: "Gecikmə göstəricisi",
    load: "Yüklənmə sərhədi",
  },
  forms: {
    newStaff: "Yeni personal yarat",
    newDoctor: "Yeni həkim yarat",
    newPatient: "Yeni pasiyent",
    patientRegistration: "Qeydiyyat formu",
    appointmentBooking: "Çakışmasız randevu yarat",
    schedule: "Yaxın təqvim",
    patientProfile: "Pasiyent profili",
    patientDetails: "Tibbi tarixçə",
    newRecord: "Yeni rapor və təyinat",
  },
  status: {
    CONFIRMED: "Təsdiqlənib",
    PENDING: "Gözləyir",
    COMPLETED: "Tamamlanıb",
    CANCELED: "Ləğv edilib",
    NO_SHOW: "Gəlməyib",
  },
  channel: {
    web: "Veb",
    mobile: "Mobil",
    "call-center": "Qeydiyyat",
  },
  confirm: {
    deleteStaff:
      "Bu işçini silmək istədiyinizə əminsiniz? Bu əməliyyat geri qaytarılmayacaq.",
    deleteDoctor:
      "Bu həkimi silmək istədiyinizə əminsiniz? Bu əməliyyat geri qaytarılmayacaq.",
  },
  messages: {
    loadingData: "Məlumatlar yüklənir...",
    loggedOut: "Çıxış edildi.",
    deleteSuccess: "Uğurla silindi.",
    dataLoadError: "Veri yüklənmədi.",
    loginError: "Giriş uğursuz oldu.",
    loginSuccess: "Giriş uğurla tamamlandı.",
    bootstrapError: "Bootstrap uğursuz oldu.",
    resetPasswordError: "Şifrə sıfırlama uğursuz oldu.",
    changePasswordError: "Şifrə dəyişdirmə uğursuz oldu.",
    registerError: "Qeydiyyat mümkün olmadı.",
    adminAccountCreated:
      "Admin hesabı yaradıldı. İndi admin girişi edə bilərsiniz.",
    patientAccountCreated: "Pasiyent hesabınız yaradıldı və giriş edildi.",
    patientCreated: "Pasiyent qeydiyyatı tamamlandı.",
    patientCreateError: "Pasiyent yaradılmadı.",
    patientAdded: "Pasiyent uğurla əlavə edildi.",
    doctorCreated: "Yeni həkim əlavə edildi.",
    doctorCreateError: "Həkim yaradılmadı.",
    doctorAdded: "Həkim uğurla əlavə edildi.",
    staffCreateError: "İşçi hesabı yaradılmadı.",
    staffAccountCreated: "{{role}} hesabı yaradıldı.",
    staffDeactivated: "İşçi hesabı deaktiv edildi.",
    deactivationError: "Deaktivasiya mumkun olmadi.",
    deleteError: "Silinmə uğursuz oldu.",
    doctorDeleted: "Həkim uğurla silindi.",
    nurseDeleted: "Hemşire uğurla silindi.",
    appointmentCreated: "Randevu yaradıldı.",
    appointmentCreateError: "Randevu yaradılmadı.",
    appointmentBooked: "Randevu sorğunuz qeydə alındı.",
    appointmentBookingError: "Randevu alına bilmədi.",
    medicalRecordCreated: "Pasiyent üçün tibbi qeyd əlavə edildi.",
    medicalRecordCreateError: "Tibbi qeyd yazılmadı.",
    medicalRecordAdded: "Pasiyent üçün tibbi qeyd əlavə edildi.",
    medicalRecordError: "Tibbi qeyd yazılmadı.",
    callCenterUserCreated: "Yeni çağrı mərkəzi istifadəçisi yaradıldı.",
    unknownError: "Naməlum xəta baş verdi.",
    logoutSuccess: "Çıxış edildi.",
  },
  doctor: {
    selectPatient: "Detalları görmək üçün sol tərəfdən bir pasiyent seçin.",
    medicalRecord: "Tibbi qeyd",
    newReport: "Yeni rapor və təyinat",
    patient: "Pasiyent",
    diagnosis: "Diaqnoz",
    treatmentPlan: "Təyinat və mücalicə planı",
    saveRecord: "Qeydi saxla",
    selectedPatient: "Seçilmiş pasiyent",
    medicalHistory: "Tibbi tarixçə",
    patientList: "Pasiyent siyahısı",
    assignedPatients: "Mənə təhkim olunmuş pasiyentlər",
    noRecord: "Hələ tibbi qeyd yoxdur",
    accountSettings: "Hesab tənzimləmələri",
    changePassword: "Şifrə dəyişdirmə",
    currentPassword: "Cari Şifrə",
    newPassword: "Yeni Şifrə",
    confirmPassword: "Şifrə təsdiqi",
    changePasswordButton: "Şifrəni dəyişdir",
    todaySchedule: "Günün cədvəli",
    myAppointments: "Randevularım",
    patientProfile: "Pasiyent profili",
    selectPatientPlaceholder: "Pasiyent seçin",
    bloodType: "Qan qrupu",
    allergies: "Allergiyalar",
    chronicConditions: "Xroniki xəstəlik",
    pastAppointments: "Keçmiş randevular",
    reportsAndPrescriptions: "Rapor və təyinatlar",
    previousVisitsAndReceipts: "Əvvəlki görüşlər və reseptlər",
    pastAndReports: "Keçmiş və raporlar",
    reportType: "Rapor növü",
    generalReport: "Ümumi hesabat",
    bloodTest: "Qan testi",
    xRay: "Rentgen",
    ultrasound: "Ultrason",
    ecg: "EKG",
    biopsy: "Biyopsiya",
    consultation: "Məsləhətləşmə",
    testResults: "Test nəticələri",
    testName: "Test adı",
    value: "Dəyər",
    unit: "Vahid",
    referenceRange: "Referans aralığı",
    status: "Status",
    normal: "Normal",
    high: "Yüksək",
    low: "Aşağı",
    critical: "Təhlükəli",
    addTestResult: "Test nəticəsi əlavə et",
    downloadReport: "Raporu yüklə",
    shareReport: "Raporu paylaş",
    shareViaEmail: "E-poçt ilə paylaş",
    shareViaWhatsApp: "WhatsApp ilə paylaş",
    pdfTemplateSettings: "PDF Şablonu Ayarları",
    hospitalName: "Xəstəxana Adı",
    hospitalAddress: "Ünvan",
    hospitalPhone: "Telefon",
    doctorTitle: "Həkim Ünvanı",
    doctorSignature: "Həkim İmza",
    hospitalLogo: "Klinik Logosu (URL)",
    footerText: "Alt Mətn",
    loadCurrentSettings: "Cari Ayarları Yüklə",
    saveSettings: "Ayarları Yadda Saxla",
    settingsSaved: "Ayarlar uğurla yadda saxlanıldı",
  },
  patient: {
    newAppointment: "Yeni randevu",
    selectDoctor: "Həkim seç və vaxt al",
    doctor: "Həkim",
    start: "Başlama",
    end: "Bitmə",
    note: "Qeyd",
    bookAppointment: "Randevu al",
    upcomingAppointments: "Gələn randevular",
    newAndActiveVisits: "Yeni və aktiv görüşlər",
    profile: "Profil",
    myMedicalInfo: "Tibbi məlumatlarım",
    gender: "Cins",
    pastAndReports: "Keçmiş və raporlar",
    previousVisitsAndReceipts: "Əvvəlki görüşlər və reseptlər",
    selectOption: "Seçin",
  },
  subscription: {
    basic: {
      name: "Basic plan",
      feature1: "5 istifadəçi limiti",
      feature2: "100 pasiyent limiti",
      feature3: "Əsas funksiyalar",
    },
    professional: {
      name: "Professional plan",
      feature1: "15 istifadəçi limiti",
      feature2: "500 pasiyent limiti",
      feature3: "Əlavə funksiyalar",
      feature4: "Prioritet dəstək",
    },
    enterprise: {
      name: "Enterprise plan",
      feature1: "50 istifadəçi limiti",
      feature2: "2000 pasiyent limiti",
      feature3: "Bütün funksiyalar",
      feature4: "Premium dəstək",
    },
  },
};
export default az;