...
export class ObservabilityStore {
  ...
  // Yeni: hər sorğunun timestamp-ı və müddəti
  private requestEvents: { timestamp: number; durationMs: number; statusCode: number }[] = [];
  ...
  recordRequest(durationMs: number, statusCode: number) {
    ...
    // Yeni: hər sorğunu yadda saxla (son MAX_RESPONSE_SAMPLES qədər)
    this.requestEvents.push({ timestamp: Date.now(), durationMs, statusCode });
    if (this.requestEvents.length > MAX_RESPONSE_SAMPLES) {
      this.requestEvents.shift();
    }
  }
  // Yeni: tarix aralığına görə statistikalar
  getMetricsByDateRange(from: Date, to: Date) {
    const fromMs = from.getTime();
    const toMs = to.getTime();
    const filtered = this.requestEvents.filter(ev => ev.timestamp >= fromMs && ev.timestamp <= toMs);
    const totalRequests = filtered.length;
    const totalErrors = filtered.filter(ev => ev.statusCode >= 500).length;
    const slowRequests = filtered.filter(ev => ev.durationMs >= 1000).length;
    const avg = filtered.length ? filtered.reduce((sum, ev) => sum + ev.durationMs, 0) / filtered.length : 0;
    const sorted = filtered.map(ev => ev.durationMs).sort((a, b) => a - b);
